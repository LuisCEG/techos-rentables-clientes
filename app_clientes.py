...
if df is None:
    ...
    if folio_input:
        ...
        if not proyecto.empty:
            ...
            # --- DOBLE FACTOR DE SEGURIDAD (CÉDULA/NIT) ---
            st.info("🔒 Por seguridad, necesitamos validar tu identidad.")
            
            # Extraemos la cédula de la base de datos y le quitamos puntos y espacios
            ...
            if cedula_input:
                # Limpiamos también lo que digita el cliente
                cedula_input_limpia = re.sub(r'[\.\,\s]', '', cedula_input.strip())
                
                # Si las contraseñas coinciden, se despliega toda la información
                if cedula_input_limpia == cedula_db_limpia and cedula_db_limpia != "vacio":
                ...
                # Si digitan la cédula mal o no coincide con la base de datos
                else:
                    st.error("❌ El documento ingresado no coincide con el registrado en este folio. Por favor comuníquese con nuestra línea de atención para actualizar sus datos.")
